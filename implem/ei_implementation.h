 ...
 *				alpha channel.
 */
int ei_impl_map_rgba(ei_surface_t surface, ei_color_t color);
 ...
 * 		The children are draw_round_rect withing the limits of the clipper and
 * 		the widget's content_rect.
 *
 * @param	widget		The widget which children are drawn.
 * @param	surface		A locked surface where to draw_round_rect the widget's children.
...
/**
 * @brief	A struct that contains the R, G, B, and Alpha channel indices of a surface.
 */
typedef struct {
    int ir, ig, ib, ia;
} color_indexes;
...
/**
 * \brief	initializes the channel indices of the root window when an application is created.
 *          The function should be called everytime we create a new application (\ref ei_app_create).
 *
 * @param	surface     The surface that we want to have the channel indices (it should be the root window).
 */
void init_color_indexes(ei_surface_t surface);


/**
 * \brief	returns the channel indices of the root window.
 *
 * @return	returns the channel indices of the root window.
 */
